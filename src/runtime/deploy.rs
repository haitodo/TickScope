...
/// Generate an EA per configured broker so attaching it requires no port or ID edits.
pub fn deploy_mt5_files_for_brokers(config: &Mt5DeployConfig, brokers: &[BrokerConfig]) -> DeployReport {
    ...
    for term in &terminals {
        ...
        let generated = deploy_broker_eas(term, &sources, brokers);
        rep.results.extend(generated);
        rep.compile_status = Some(compile_deployed_ea(&rep, &term.mql5_dir.join("Experts/TickCollector.mq5")));
        for broker in brokers {
            let name = broker_ea_name(broker);
            let status = compile_deployed_ea(&rep, &term.mql5_dir.join("Experts/TickScope").join(&name));
            rep.broker_compile_results.push((name, status));
        }
...
pub fn print_deploy_report(report: &DeployReport) {
    ...
    if report.terminals.is_empty() {
        ...
        for term in &report.terminals {
            ...
            for (name, status) in &term.broker_compile_results {
                println!("    - {}: {:?}", name, status);
            }
        }
    }
